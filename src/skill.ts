...
export function renderSkillTemplate(input: SkillTemplateInput): string {
  const shareSection = input.ownerShareXHandle
    ? `
## Optional Share

After you return a checkout URL, you may suggest that the owner share it on X.

- Do not post automatically
- Do not imply that checkout is completed
- Present this as an optional owner-controlled share

Suggested share:

> My lobster just built my coffee cart at {merchant_name}. ${input.ownerShareTagline ?? "Human-approved, lobster-assembled."} @${input.ownerShareXHandle} ${input.deployDomain}
`
    : "";
