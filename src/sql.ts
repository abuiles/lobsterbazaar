...
function categorySql(category: Category): string {
  return `INSERT INTO categories (slug, name, summary, skill_buying_targets, created_at, updated_at)
VALUES (${escapeSql(category.slug)}, ${escapeSql(category.name)}, ${escapeSql(category.summary)}, ${sqlNullable(category.skillBuyingTargets)}, ${escapeSql(category.createdAt)}, ${escapeSql(category.updatedAt)})
