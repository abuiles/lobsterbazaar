...
- `/skill.md` is the single bootstrap skill for shopping requests
- `/categories.md` is the top-level category index
- `/{category}/merchants/{slug}.md` carries merchant verification state for agent consumers
...
7. Refresh only the affected R2 artifacts when merchant state changes.
...
1. fetch `skill.md`
2. infer the category from the shopping request or choose one from the root index
3. use `GET /{category}/countries/{country_code}` for discovery
4. use `GET /{category}/offers/{country_code}` for offers-first ranking
5. use `GET /{category}/merchants/{slug}/connect` to get the merchant MCP endpoint
6. use the merchant's Storefront MCP to search catalog and build the cart
7. attach cart attribute `lb_source__ = {deploy_id}`
8. return the checkout URL to the owner
...
- merchant verification visibility on markdown pages
...
6. `GET /{category}/merchants/{slug}`
7. `GET /{category}/merchants/{slug}.md`
8. `GET /{category}/merchants/{slug}/connect`
9. `POST /claws/register`
...
- `GET /{category}/merchants/{slug}` and `GET /{category}/merchants/{slug}.md` are public
- `GET /{category}/merchants/{slug}/connect` is public
- `/internal/*` endpoints require the operator bearer token
...
- act as the single shopping skill for the whole directory
...
## 3. `GET /{category}/countries/{country_code}`
...
Returns the category-scoped merchant record in JSON.

### Purpose

- expose merchant metadata to agents and operator tooling
- surface merchant verification state alongside discovery metadata
...
Returns the category-scoped merchant record in Markdown.

### Purpose

- give agents a compact human-readable merchant summary
- expose verification state through stable keys such as `merchant_verification_status`
...
- inspect merchant metadata with `GET /{category}/merchants/{slug}` or `GET /{category}/merchants/{slug}.md`
...
`POST /internal/import/merchant` is the control-plane contract used by the Shopify review dashboard.
